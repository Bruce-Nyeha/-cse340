...
/**
 * Fetch all categories from the index table
 * 🚀 FIXED: Added the missing explicit export for projects controller mapping
 */
...
export const getCategoryById = async (categoryId) => {
    ...
    return result.rows[0]; // 🚀 FIXED: Returns the single row object immediately
};
...
/**
 * Fetch all projects belonging to a single category tracking loop
 */
...
/**
 * Update an existing category name string record
 */
...
/**
 * Manage structural assignments inside the checkbox relational junction table
 */
export const updateProjectCategories = async (projectId, categoryIds) => {
    await query('DELETE FROM project_category WHERE project_id = $1;', [projectId]);
    if (categoryIds) {
        const ids = Array.isArray(categoryIds) ? categoryIds : [categoryIds];
        for (const catId of ids) {
            await query('INSERT INTO project_category (project_id, category_id) VALUES ($1, $2);', [projectId, catId]);
        }
    }
};
